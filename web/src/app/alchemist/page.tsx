    ...
    <div className="flex flex-col items-center justify-center py-10 overflow-hidden relative min-h-[350px]">
      <div className="relative w-64 h-64 flex items-center justify-center">
        ...
        <div className={`absolute inset-0 border-2 border-dashed rounded-full transition-all duration-1000 ${
          isBrewing ? "border-amber-500 animate-spin" : "border-stone-200 animate-spin"
        ...
        <div className="relative z-10 flex flex-col items-center transition-transform duration-500 hover:scale-105">
            {/* Head */}
            <div className={`w-24 h-24 bg-stone-900 rounded-full border-4 shadow-2xl flex items-center justify-center relative overflow-hidden transition-all duration-500 ${
              isBrewing ? "border-amber-500 scale-110" : "border-stone-800"
                ...
                <div className="flex gap-3">
                    ...
                    <div className="w-6 h-6 border-2 border-white/10 rounded-full flex items-center justify-center bg-stone-800 overflow-hidden">
                        <div className={`w-1.5 h-1.5 bg-white rounded-full transition-all duration-300 ${
                            isIdle ? "animate-eye-blink" : ""
                        } ${isAdding ? "scale-[1.5] bg-amber-400" : ""} ${
                            isBrewing ? "scale-[1.8] shadow-[0_0_12px_white] animate-pulse" : ""
                        } ${isSuccess ? "scale-[1.3] bg-green-400 rotate-45 rounded-none w-2 h-0.5" : ""} ${
                            state === 'error' ? "scale-[1.2] bg-red-500 rotate-45 w-3 h-0.5" : ""
                        }`} />
                    </div>
                    {/* Right Eye */}
                    <div className="w-6 h-6 border-2 border-white/10 rounded-full flex items-center justify-center bg-stone-800 overflow-hidden">
                ...
                <div className={`absolute inset-0 transition-opacity duration-1000 ${
                    isBrewing ? "bg-amber-500/20 opacity-100" : "bg-transparent opacity-0"
                } ${isSuccess ? "bg-green-500/20 opacity-100" : ""}`} />
            </div>
            {/* Body / Cape */}
            <div className={`w-28 h-14 bg-stone-900 rounded-t-full -mt-2 shadow-lg border-x-4 border-t-4 transition-colors duration-500 ${
                isBrewing ? "border-amber-500/50" : "border-stone-800/50"
            } ${isSuccess ? "border-green-500/50" : ""}`} />
        </div>

        {/* Hand & Beaker */}
        <div className={`absolute top-24 right-4 z-20 transition-all duration-500 ${
            isBrewing ? "animate-alchemist-shake" : "animate-alchemist-bounce"
        }`}>
                ...
                <div className="absolute -top-8 -left-4 -rotate-12 transition-transform hover:scale-110">
                    <FlaskConical 
                        ...
                        className={`transition-all duration-500 ${
                            isBrewing ? "text-amber-500 fill-amber-500/40" : "text-stone-300 fill-stone-100"
                    ...
                    <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-8 overflow-hidden pointer-events-none">
                        <div 
                            className={`w-full transition-all duration-700 rounded-t-sm ${
                                isBrewing ? "bg-amber-400" : "bg-stone-300"
                            ...
                            <div key={i} className="flex items-center justify-between bg-white/5 border border-white/5 hover:border-white/20 rounded-2xl p-4 group animate-in slide-in-from-left-4 duration-300">
                                <div className="flex items-center gap-4">
                                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold ${
                                        step.type === 'add' ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"
                                    }`}>
